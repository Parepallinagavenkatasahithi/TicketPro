...
def test_subsystem_55_module1_execution():
    service = AutomationRulesServiceModule1(service_id=55)
    assert service.service_id == 55
    assert service.subsystem_name == "automation_rules"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-55-01",
        entity_id=155,
...
def test_subsystem_55_access_control():
    service = AutomationRulesServiceModule2(service_id=55)
...
def test_subsystem_55_export_transformation():
    service = AutomationRulesServiceModule3(service_id=55)
    records = [
