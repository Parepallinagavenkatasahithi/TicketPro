...
def test_subsystem_21_module1_execution():
    service = ExecutiveDashboardServiceModule1(service_id=21)
    assert service.service_id == 21
    assert service.subsystem_name == "executive_dashboard"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-21-01",
        entity_id=121,
...
def test_subsystem_21_access_control():
    service = ExecutiveDashboardServiceModule2(service_id=21)
...
def test_subsystem_21_export_transformation():
    service = ExecutiveDashboardServiceModule3(service_id=21)
    records = [
