...
def test_subsystem_61_module1_execution():
    service = AssetCMDBServiceModule1(service_id=61)
    assert service.service_id == 61
    assert service.subsystem_name == "asset_cmdb"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-61-01",
        entity_id=161,
...
def test_subsystem_61_access_control():
    service = AssetCMDBServiceModule2(service_id=61)
...
def test_subsystem_61_export_transformation():
    service = AssetCMDBServiceModule3(service_id=61)
    records = [
