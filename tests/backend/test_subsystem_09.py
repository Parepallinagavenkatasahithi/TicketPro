...
def test_subsystem_9_module1_execution():
    service = EmployeeDirectoryServiceModule1(service_id=9)
    assert service.service_id == 9
    assert service.subsystem_name == "employee_directory"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-9-01",
        entity_id=109,
...
def test_subsystem_9_access_control():
    service = EmployeeDirectoryServiceModule2(service_id=9)
...
def test_subsystem_9_export_transformation():
    service = EmployeeDirectoryServiceModule3(service_id=9)
    records = [
