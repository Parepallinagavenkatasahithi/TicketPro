...
def test_subsystem_64_module1_execution():
    service = IncidentResponseServiceModule1(service_id=64)
    assert service.service_id == 64
    assert service.subsystem_name == "incident_response"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-64-01",
        entity_id=164,
...
def test_subsystem_64_access_control():
    service = IncidentResponseServiceModule2(service_id=64)
...
def test_subsystem_64_export_transformation():
    service = IncidentResponseServiceModule3(service_id=64)
    records = [
