...
def test_subsystem_72_module1_execution():
    service = AuditComplianceServiceModule1(service_id=72)
    assert service.service_id == 72
    assert service.subsystem_name == "audit_compliance"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-72-01",
        entity_id=172,
...
def test_subsystem_72_access_control():
    service = AuditComplianceServiceModule2(service_id=72)
...
def test_subsystem_72_export_transformation():
    service = AuditComplianceServiceModule3(service_id=72)
    records = [
