...
def test_subsystem_13_module1_execution():
    service = ProblemMgmtServiceModule1(service_id=13)
    assert service.service_id == 13
    assert service.subsystem_name == "problem_mgmt"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-13-01",
        entity_id=113,
...
def test_subsystem_13_access_control():
    service = ProblemMgmtServiceModule2(service_id=13)
...
def test_subsystem_13_export_transformation():
    service = ProblemMgmtServiceModule3(service_id=13)
    records = [
