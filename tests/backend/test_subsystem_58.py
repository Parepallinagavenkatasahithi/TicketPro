...
def test_subsystem_58_module1_execution():
    service = KnowledgeBaseServiceModule1(service_id=58)
    assert service.service_id == 58
    assert service.subsystem_name == "knowledge_base"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-58-01",
        entity_id=158,
...
def test_subsystem_58_access_control():
    service = KnowledgeBaseServiceModule2(service_id=58)
...
def test_subsystem_58_export_transformation():
    service = KnowledgeBaseServiceModule3(service_id=58)
    records = [
