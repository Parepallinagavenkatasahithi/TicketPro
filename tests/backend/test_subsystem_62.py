...
def test_subsystem_62_module1_execution():
    service = ChangeMgmtServiceModule1(service_id=62)
    assert service.service_id == 62
    assert service.subsystem_name == "change_mgmt"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-62-01",
        entity_id=162,
...
def test_subsystem_62_access_control():
    service = ChangeMgmtServiceModule2(service_id=62)
...
def test_subsystem_62_export_transformation():
    service = ChangeMgmtServiceModule3(service_id=62)
    records = [
