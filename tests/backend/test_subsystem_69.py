...
def test_subsystem_69_module1_execution():
    service = ReportingEngineServiceModule1(service_id=69)
    assert service.service_id == 69
    assert service.subsystem_name == "reporting_engine"
    
    res = service.process_subsystem_transaction_1(
        transaction_id="TX-TEST-69-01",
        entity_id=169,
...
def test_subsystem_69_access_control():
    service = ReportingEngineServiceModule2(service_id=69)
...
def test_subsystem_69_export_transformation():
    service = ReportingEngineServiceModule3(service_id=69)
    records = [
